...
class BaseModel(ABC):
    ...
    def forward_semantic_mask(self):
        ...
        if self.opt.f_s_net == "sam":
            ...
            self.input_A_label_mask = torch.clone(self.pred_f_s_real_A).to(self.device)
            self.gt_pred_f_s_real_A_max = (self.input_A_label_mask > 0.0).float()
        else:
            if self.opt.train_mask_disjoint_f_s:
                f_s = self.netf_s_A
            else:
        ...
        if self.opt.f_s_net == "sam":
            ...
            self.input_B_label_mask = torch.clone(self.pred_f_s_real_B).to(self.device)
